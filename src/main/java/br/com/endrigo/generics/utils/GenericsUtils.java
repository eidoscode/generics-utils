 ...
 * This will help you if your class is going to be executed throw a proxy execution
 * such as EJB.<br/>
 * 
 * @author eantonini
 * @since 1.0
...
public final class GenericsUtils {
	...
	public static <X, T> Class<T> getSuperClassGenericType(
		...
		if (index < 0) {
			throw new IllegalArgumentException(
					"The index value can not be less than 0.");
		}

		Class<T> retValue = null;
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Original class: \'" + clazz.getName()
					+ "\' Looking for an class that extends this class: \'"
					+ clazzType.getName() + "\'");
		}
		Class<?> retClazz = getClass(clazz, clazzType);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Found the class: \'"
					+ ((retClazz != null) ? retClazz.getName() : "null")
					+ "\' that extends this class: \'" + clazzType.getName()
					+ "\'");
		}
	...
	/**
	 * Keep looking for the super class until it finds the implementation class
	 * of the class type sent by parameter.
	 * 
	 * @param clazz
